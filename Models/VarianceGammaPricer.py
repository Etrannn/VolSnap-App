...
from prereq import fft_Lewis, IV_from_Lewis, Q1, Q2,Gil_Pelaez_pdf,VG_pdf
...
class VG_pricer:
    ...
    def plot_IV_surface(self, K_range, T_range,num_K=10,num_T=10):
        ...
        ax.set_title('Variance Gamma Implied Volatility Surface')
    ...
    def plot_density(self,N=100000):
        G = ss.gamma(self.T / self.kappa, scale=self.kappa).rvs(N)  # The gamma RV
        Norm = ss.norm.rvs(0, 1, N)  # The normal RV
        X = self.theta * G + self.sigma * np.sqrt(G) * Norm
        cf_VG_b = partial(cf_VG, t=self.T, mu=0, theta=self.theta, sigma=self.sigma, kappa=self.kappa)
        x = np.linspace(X.min(), X.max(), 500)
        y = np.linspace(-2, 1, 30)

        fig = plt.figure(figsize=(16, 5))
        ax = fig.add_subplot(111)
        ax.plot(x, VG_pdf(x, self.T, 0, self.theta, self.sigma, self.kappa), color="r", label="VG density")
        ax.plot(y, [Gil_Pelaez_pdf(i, cf_VG_b, np.inf) for i in y], "p", label="Fourier inversion")
        ax.hist(X, density=True, bins=200, facecolor="LightBlue", label="frequencies of X")
        ax.legend()
        ax.set_title("Variance Gamma Histogram")
        return fig
    ...
    def plot_qq(self,N=100000):
        ...
        ax.set_title("Variance Gamma Q-Q Plot")
